...
interface SignUpPageProps {
  onSignUp: (username: string, email: string, password: string) => string | null;
  onGoLogin: () => void;
  onGuest: () => void;
}

export const SignUpPage: React.FC<SignUpPageProps> = ({ onSignUp, onGoLogin, onGuest }) => {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const err = onSignUp(username.trim(), email.trim(), password);
    if (err) setError(err);
  }
      ...
      <div className="w-full max-w-md space-y-8">
        ...
        <div className="space-y-2">
          ...
          <p className="text-[13px] text-[#c3b7a4] leading-relaxed">
            Save your picks and build a fragrance collection that's yours.
          </p>
        </div>
